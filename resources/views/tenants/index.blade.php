...
<x-table><tbody>@foreach($tenants as $tenant)<tr class="border-t"><td class="p-4 font-medium">{{ $tenant->full_name }}</td><td class="p-4">{{ $tenant->phone }}</td><td class="p-4"><a class="tap-target inline-flex items-center rounded border px-3 text-slate-700" href="{{ route('tenants.show', $tenant) }}">View</a></td></tr>@endforeach</tbody></x-table>
{{ $tenants->links() }}
@endsection

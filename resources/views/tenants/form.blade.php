...
<form data-tenant-form method="post" action="{{ $tenant->exists ? route('tenants.update', $tenant) : route('tenants.store') }}" class="grid max-w-2xl gap-4 rounded border bg-brand-surface p-4 shadow-sm md:grid-cols-2">
...
<div class="grid gap-2 md:col-span-2 sm:grid-cols-2">
    <button class="tap-target min-h-11 w-full rounded bg-brand-primary px-4 text-white">{{ __('app.actions.save') }}</button>
    @if(! $tenant->exists)
        @can('create', \App\Models\Contract::class)
            <button type="submit" name="after_save" value="create_contract" class="tap-target min-h-11 w-full rounded border border-brand-border bg-brand-surface px-4 text-brand-text">{{ __('tenants.actions.save_and_create_contract') }}</button>
        @endcan
    @endif
</div>
</form>

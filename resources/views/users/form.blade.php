...
<label class="block text-sm font-medium">{{ __('users.fields.role') }} <select name="role" class="tap-target mt-1 w-full rounded border p-2">@foreach(['owner','manager','accountant','caretaker'] as $role)<option value="{{ $role }}" @selected(old('role', optional($user->role)->value)==$role)>{{ __('users.roles.'.$role) }}</option>@endforeach</select></label>

    ...
    <div class="flex flex-wrap gap-2">
        @can('create', \App\Models\Contract::class)
            @if($contract->isRenewalEligible())
                <a class="rounded bg-slate-900 px-3 py-2 text-sm text-white" href="{{ route('contracts.create', ['renew_from' => $contract->id]) }}">Prepare renewal</a>
            @endif
        @endcan
        <a class="rounded border px-3 py-2 text-sm" href="{{ route('contracts.pdf', $contract) }}">Download contract PDF</a>
        <a class="rounded border px-3 py-2 text-sm" href="{{ route('contracts.edit', $contract) }}">Edit contract</a>
    </div>
</div>

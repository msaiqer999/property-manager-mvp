...
<div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
    <div>
        ...
        <p class="mt-1 text-sm text-brand-muted">{{ __('contracts.show_description') }}</p>
    </div>
    <div class="grid gap-2 sm:flex sm:flex-wrap">
        @can('create', \App\Models\Contract::class)
            @if($contract->isRenewalEligible())
                <a class="tap-target inline-flex min-h-11 items-center justify-center rounded bg-brand-primary px-4 text-center text-sm font-medium text-white" href="{{ route('contracts.create', ['renew_from' => $contract->id]) }}">{{ __('contracts.show.prepare_renewal') }}</a>
            @endif
        @endcan
...
@if(session('status') && auth()->user()?->role?->can('view-payments'))
    <div data-contract-created-guidance class="mb-4 rounded border border-state-success/25 bg-state-success-soft p-4 text-sm text-state-success">
        <div class="grid gap-3 sm:flex sm:items-center sm:justify-between">
            <p>{{ __('contracts.show.created_guidance') }}</p>
            <a class="tap-target inline-flex min-h-11 items-center justify-center rounded bg-brand-primary px-4 text-center text-sm font-medium text-white" href="{{ route('payments.index') }}">{{ __('contracts.show.view_payments') }}</a>
        </div>
    </div>
@endif

@if($contract->daysUntilExpiry() !== null && $contract->daysUntilExpiry() <= 90)
    <div class="mb-4 rounded border border-state-warning/30 bg-state-warning-soft p-3 text-sm text-state-warning">
...
<div data-contract-show-card class="grid gap-3 rounded border bg-brand-surface p-4 shadow-sm sm:grid-cols-2">
    ...
    <p><span class="font-medium">{{ __('contracts.show.status') }}</span> <x-status-badge :status="$contract->status" :label="__('contracts.statuses.'.$contract->status)" /></p>
</div>

@if($contract->status === 'terminated')
    <div class="mt-4 grid gap-3 rounded border border-state-warning/30 bg-state-warning-soft p-4 text-sm text-state-warning sm:grid-cols-2">
...
@elseif(auth()->user()?->can('terminate', $contract) && $contract->status === 'active' && $contract->end_date->toDateString() >= now()->toDateString())
    <form method="post" action="{{ route('contracts.terminate', $contract) }}" class="mt-4 space-y-3 rounded border border-state-warning/30 bg-state-warning-soft p-4">
        @csrf
        @method('patch')
        <label class="block text-sm font-medium">{{ __('contracts.lifecycle.termination_reason') }}
            ...
            @error('termination_reason')<span class="mt-1 block text-sm text-state-danger">{{ $message }}</span>@enderror
        </label>
        <p class="text-sm text-state-warning">{{ __('contracts.lifecycle.confirm_terminate') }}</p>
        <button class="tap-target rounded bg-state-danger px-4 text-sm text-white">{{ __('contracts.lifecycle.terminate') }}</button>
    </form>
@endif
...
<div data-contract-payments-mobile-list class="grid gap-3 md:hidden">
    @foreach($contract->payments as $payment)
        <article data-contract-payment-mobile-card class="rounded border bg-brand-surface p-4 shadow-sm">
            <div class="flex items-start justify-between gap-3">
                <div>
                    <p class="text-sm text-brand-muted">{{ __('payments.columns.due_date') }}</p>
                    <p class="font-semibold"><bdi dir="ltr">{{ $payment->due_date->toDateString() }}</bdi></p>
                </div>
                <x-status-badge :status="$payment->status" :label="__('payments.statuses.'.$payment->status)" />
            </div>
            <p class="mt-3 text-sm text-brand-muted">{{ __('payments.columns.amount') }}: <bdi dir="ltr">{{ number_format($payment->amount_due, 2) }}</bdi></p>
            @if($payment->status === 'cancelled')
                ...
                @else
                    <span class="mt-3 block text-sm text-brand-muted">{{ __('payments.lifecycle.cancelled_due_to_contract_termination') }}</span>
                @endif
            ...
            @elseif(in_array($payment->status, ['pending', 'partial', 'overdue'], true))
                <a class="tap-target mt-4 inline-flex min-h-11 w-full items-center justify-center rounded bg-brand-primary px-4 text-center text-sm font-medium text-white" href="{{ route('payments.edit', $payment) }}">{{ __('contracts.show.record_payment') }}</a>
            @else
        ...
        @foreach($contract->payments as $payment)
            <tr class="border-t">
                ...
                <td class="p-3 whitespace-nowrap">
                    <x-status-badge :status="$payment->status" :label="__('payments.statuses.'.$payment->status)" />
                    @if($payment->status === 'cancelled')
                        <span class="block text-xs text-brand-muted">{{ __('payments.lifecycle.cancelled_due_to_contract_termination') }}</span>
                    @endif
                </td>
                <td class="p-3 whitespace-nowrap">
                    @if($payment->status === 'cancelled')
                        @if($payment->amount_paid > 0)
                            <a class="tap-target inline-flex items-center text-brand-primary" href="{{ route('payments.receipt', $payment) }}">{{ __('payments.view_receipt') }}</a>
                        @else
                            <span class="text-sm text-brand-muted">{{ __('payments.lifecycle.cancelled_due_to_contract_termination') }}</span>
                        @endif
                    @elseif($payment->status === 'paid')
                        <a class="tap-target inline-flex items-center text-brand-primary" href="{{ route('payments.show', $payment) }}">{{ __('payments.view_receipt') }}</a>
                    @elseif(in_array($payment->status, ['pending', 'partial', 'overdue'], true))
                        <a class="tap-target inline-flex items-center text-brand-primary" href="{{ route('payments.edit', $payment) }}">{{ __('contracts.show.record_payment') }}</a>
                    @else
                        <a class="tap-target inline-flex items-center text-brand-primary" href="{{ route('payments.show', $payment) }}">{{ __('app.actions.view') }}</a>
                    @endif
                </td>
            </tr>
        @endforeach

...
@else
<label class="block text-sm font-medium">{{ __('contracts.form.unit') }} <select name="unit_id" class="form-select-safe tap-target mt-1 w-full rounded border p-2">@foreach($units as $unit)<option value="{{ $unit->id }}" @selected(old('unit_id', $contract->unit_id)==$unit->id)>{{ $unit->building->name }} / {{ $unit->unit_number }} - {{ $unit->availability_label }}</option>@endforeach</select><span class="mt-1 block text-xs text-slate-500">{{ __('contracts.form.unit_hint') }}</span></label>
@endif

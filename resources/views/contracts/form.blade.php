@extends('layouts.app')
@section('content')
<h1 class="mb-4 text-xl font-semibold">{{ $contract->exists ? 'Edit contract' : 'Add contract' }}</h1>
...
<label class="block text-sm font-medium">Tenant <select name="tenant_id" class="tap-target mt-1 w-full rounded border p-2">@foreach($tenants as $tenant)<option value="{{ $tenant->id }}" @selected(old('tenant_id', $contract->tenant_id)==$tenant->id)>{{ $tenant->full_name }}</option>@endforeach</select></label>
<label class="block text-sm font-medium">Unit <select name="unit_id" class="tap-target mt-1 w-full rounded border p-2">@foreach($units as $unit)<option value="{{ $unit->id }}" @selected(old('unit_id', $contract->unit_id)==$unit->id)>{{ $unit->building->name }} / {{ $unit->unit_number }}</option>@endforeach</select></label>
...
<label class="block text-sm font-medium">Rent <input name="rent_amount" type="number" step="0.01" value="{{ old('rent_amount', $contract->rent_amount) }}" class="tap-target mt-1 w-full rounded border p-2"></label>
<label class="block text-sm font-medium">Frequency <select name="payment_frequency" class="tap-target mt-1 w-full rounded border p-2">@foreach(['monthly','quarterly','semi_annual','annual'] as $frequency)<option @selected(old('payment_frequency', $contract->payment_frequency)==$frequency)>{{ $frequency }}</option>@endforeach</select></label>
<label class="block text-sm font-medium">Deposit <input name="deposit_amount" type="number" step="0.01" value="{{ old('deposit_amount', $contract->deposit_amount) }}" class="tap-target mt-1 w-full rounded border p-2"></label>

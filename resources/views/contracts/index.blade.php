...
<div class="mb-4 grid gap-3 sm:flex sm:items-start sm:justify-between">
    ...
    <a class="tap-target flex items-center justify-center rounded bg-slate-900 px-4 text-sm font-medium text-white" href="{{ route('contracts.create') }}">{{ __('contracts.add') }}</a>
</div>

<form class="mb-4 grid gap-3 rounded border bg-white p-3 sm:grid-cols-[1fr_auto]">
    <select name="status" class="tap-target rounded border p-2">
    ...
    <button class="tap-target rounded bg-slate-900 px-4 text-white">{{ __('contracts.filter') }}</button>
</form>

...
<label class="block text-sm font-medium">{{ __('units.fields.type') }} <select name="type" class="form-select-safe tap-target mt-1 w-full rounded border p-2">@foreach(['apartment','shop','office','warehouse','villa','chalet','other'] as $type)<option value="{{ $type }}" @selected(old('type', $unit->type)==$type)>{{ __('units.types.'.$type) }}</option>@endforeach</select></label>

        ...
        <tbody>
            @foreach($rows as $index => $row)
                <tr class="border-t">
                    ...
                    <td class="p-2"><select name="units[{{ $index }}][type]" class="tap-target w-40 rounded border p-2">@foreach($types as $type)<option value="{{ $type }}" @selected(old("units.$index.type", $row['type']) === $type)>{{ __('units.types.'.$type) }}</option>@endforeach</select></td>
                    ...
                    <td class="p-2"><select name="units[{{ $index }}][status]" class="tap-target w-40 rounded border p-2">@foreach($statuses as $status)<option value="{{ $status }}" @selected(old("units.$index.status", $row['status']) === $status)>{{ __('units.statuses.'.$status) }}</option>@endforeach</select></td>

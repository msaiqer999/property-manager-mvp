...
<form method="post" action="{{ route('buildings.units.bulk.preview', $building) }}" class="grid max-w-3xl gap-4 rounded border bg-white p-4 shadow-sm md:grid-cols-2">
    ...
    <label class="block text-sm font-medium">{{ __('units.bulk.default_type') }} <select name="type" class="tap-target mt-1 w-full rounded border p-2">@foreach($types as $type)<option value="{{ $type }}" @selected(old('type', 'apartment') === $type)>{{ __('units.types.'.$type) }}</option>@endforeach</select></label>
    ...
    <label class="block text-sm font-medium">{{ __('units.bulk.default_status') }} <select name="status" class="tap-target mt-1 w-full rounded border p-2">@foreach($statuses as $status)<option value="{{ $status }}" @selected(old('status', 'vacant') === $status)>{{ __('units.statuses.'.$status) }}</option>@endforeach</select></label>

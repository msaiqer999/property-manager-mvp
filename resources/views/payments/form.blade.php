...
<div data-payment-summary class="mb-4 rounded border bg-brand-surface p-4 shadow-sm">
...
<form data-payment-record-form method="post" enctype="multipart/form-data" action="{{ route('payments.update', $payment) }}" class="max-w-xl space-y-4 rounded border bg-brand-surface p-4 shadow-sm">
    @csrf @method('put')
    <p class="text-sm text-brand-muted">{{ __('payments.form.due') }} <span dir="ltr">{{ $payment->due_date->toDateString() }}</span> / <span dir="ltr">{{ number_format($payment->amount_due, 2) }}</span></p>
    ...
    <button data-payment-action class="tap-target min-h-11 w-full rounded bg-brand-primary px-4 text-white sm:w-auto">{{ __('payments.form.save') }}</button>
</form>
@endsection

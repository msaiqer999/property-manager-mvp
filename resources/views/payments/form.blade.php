...
<form data-payment-record-form method="post" enctype="multipart/form-data" action="{{ route('payments.update', $payment) }}" class="max-w-xl space-y-4 rounded border bg-white p-4 shadow-sm">
    ...
    <label class="block text-sm font-medium">{{ __('payments.form.amount_paid') }} <input name="amount_paid" type="number" step="0.01" value="{{ old('amount_paid', $payment->amount_paid) }}" class="tap-target mt-1 min-h-11 w-full rounded border p-3"></label>
    <label class="block text-sm font-medium">{{ __('payments.form.payment_date') }} <input name="payment_date" type="date" value="{{ old('payment_date', optional($payment->payment_date)->toDateString()) }}" class="tap-target mt-1 min-h-11 w-full rounded border p-3"></label>
    <label class="block text-sm font-medium">{{ __('payments.form.method') }} <select name="payment_method" class="tap-target mt-1 min-h-11 w-full rounded border p-3"><option value=""></option>@foreach(['cash','bank_transfer','cheque','other'] as $method)<option value="{{ $method }}" @selected(old('payment_method', $payment->payment_method)==$method)>{{ __('payments.methods.'.$method) }}</option>@endforeach</select></label>

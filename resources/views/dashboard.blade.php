...
<div class="mt-6 grid gap-4 lg:grid-cols-3">
    <section class="rounded border bg-white p-4 shadow-sm">
        ...
        <div class="mb-3 grid grid-cols-3 gap-2 text-center text-sm">
            <div class="rounded bg-slate-50 p-2"><p class="bidi-isolate font-semibold" dir="ltr">{{ $expiryCounts['30'] }}</p><p class="text-xs text-slate-500">{{ __('app.dashboard.days', ['count' => 30]) }}</p></div>
            <div class="rounded bg-slate-50 p-2"><p class="bidi-isolate font-semibold" dir="ltr">{{ $expiryCounts['60'] }}</p><p class="text-xs text-slate-500">{{ __('app.dashboard.days', ['count' => 60]) }}</p></div>
            <div class="rounded bg-slate-50 p-2"><p class="bidi-isolate font-semibold" dir="ltr">{{ $expiryCounts['90'] }}</p><p class="text-xs text-slate-500">{{ __('app.dashboard.days', ['count' => 90]) }}</p></div>
        </div>
        @forelse($expiringSoon as $contract)
            <div class="border-t py-3 text-sm">
                ...
                <p class="text-slate-600">{{ $contract->tenant->full_name }} · {{ $contract->unit->building->name }} / <bdi>{{ $contract->unit->unit_number }}</bdi></p>
        ...
        @empty <p class="text-sm text-slate-500">{{ __('app.dashboard.no_expiring_contracts') }}</p> @endforelse
    </section>
    <section class="rounded border bg-white p-4 shadow-sm">
        ...
        @empty <p class="text-sm text-slate-500">{{ __('app.dashboard.no_payments') }}</p> @endforelse
    </section>
    <section class="rounded border bg-white p-4 shadow-sm">
